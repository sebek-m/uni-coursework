 ...
 */
struct Map {
    City *firstCity; /**< Lista miast */
    WholeRoute *firstRoute; /**< Lista dróg krajowych */
    RoadSeg *firstRoad; /**< Lista odcinków drogi */
};
...
/** @brief Tworzy drogę krajową na podstawie jej opisu.
 * Jeśli to możliwe, tworzy drogę krajową o zadanym przebiegu,
 * który wcześniej został przepisany z wejścia na listę struktur CmdElement.
 * Format opisu jest taki sam jak wynik funkcji getRouteDescription.
 *
 * @param map               - wskaźnik na mapę;
 * @param routeId           - numer oczekiwanej drogi krajowej;
 * @param cmd               - lista elementów polecenia z wejścia;
 * @return Wartość @p true, jeśli się udało, @p false jeśli wystąpił błąd.
 */
 ...
 * Wartość @p false, jeśli wystąpił błąd.
 */
